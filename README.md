# <img src="https://img.icons8.com/fluency/48/lightning-bolt.png" width="36" align="center" alt="Lightning"/> Supratim Mondal

**EC Engineer**  
<img src="https://img.icons8.com/fluency/48/marker.png" width="16" align="center" alt="Location"/> Sainthia / Durgapur, India | <img src="https://img.icons8.com/fluency/48/new-post.png" width="16" align="center" alt="Email"/> mail@smrk.in | <img src="https://img.icons8.com/fluency/48/domain.png" width="16" align="center" alt="Website"/> [Website](https://smrk.in)

<p align="left">
  <a href="https://github.com/supratimrk"><img src="https://img.icons8.com/fluency/48/github.png" width="32" alt="GitHub"/></a>
  <a href="https://twitter.com/supratimrk"><img src="https://img.icons8.com/fluency/48/twitter.png" width="32" alt="X"/></a>
  <a href="https://smrk.in"><img src="https://img.icons8.com/fluency/48/domain.png" width="32" alt="Website"/></a>
</p>

---

## <img src="https://img.icons8.com/fluency/48/source-code.png" width="28" align="center" alt="Developer"/> About

Final-year B.Tech student in ECE at Dr. B. C. Roy Engineering College (batch 2026). I specialize in designing embedded systems, telecommunications infrastructure, and resilient network architectures. Recent work includes wireless smart energy telemetry, custom MPPT solar controllers, and IoT network deployment. Interned at BSNL (telecom) and Tata Power SDI.

---

## <img src="https://img.icons8.com/fluency/48/graduation-cap.png" width="28" align="center" alt="Graduation"/> Education & Experience

```mermaid
timeline
  title Academic and Professional Path
    2022-2026 : B.Tech in Electronics & Communication
              : Dr. B. C. Roy Engineering College, MAKAUT
    Summer 2025 : Telecommunications Intern
         : BSNL, Burdwan
    Winter 2026 : Supervising Engineer Trainee
         : Tata Power SDI, Maithon
```

---

## <img src="https://img.icons8.com/fluency/48/rocket.png" width="28" align="center" alt="Rocket"/> Projects

```mermaid
flowchart TB
  P[Engineering Projects Portfolio]
  T[Telemetry and Monitoring\nEnvironmental sensor aggregation]
  A[Autonomous Hardware\nCustom drone and rover logic]
  E[Power Electronics\nMPPT solar charge controller]
  I[IoT Architecture\nLoRa and MQTT smart energy network]

  P --> T
  P --> A
  P --> E
  P --> I

  classDef teal fill:#dbeafe,stroke:#0284c7,color:#0f172a;
  classDef green fill:#dcfce7,stroke:#16a34a,color:#0f172a;
  classDef amber fill:#fef3c7,stroke:#d97706,color:#0f172a;
  classDef rose fill:#ffe4e6,stroke:#e11d48,color:#0f172a;

  class P teal;
  class T green;
  class A amber;
  class E rose;
  class I teal;
```

**System Architecture: Wireless Smart Energy Network**
```mermaid
flowchart LR
  subgraph Edge[Edge Layer]
    M[Smart Meter] --> N[ESP32 Node]
  end

  N -->|MQTT / Wi-Fi| G[IoT Gateway]
  G -->|Secure TCP/IP| D[(Time-Series DB)]
  D -->|REST API| A[Network Dashboard]

  classDef edge fill:#dbeafe,stroke:#2563eb,color:#0f172a;
  classDef core fill:#dcfce7,stroke:#16a34a,color:#0f172a;
  classDef storage fill:#fef3c7,stroke:#d97706,color:#0f172a;
  classDef ui fill:#ffe4e6,stroke:#e11d48,color:#0f172a;

  class M,N edge;
  class G core;
  class D storage;
  class A ui;
```

---

## <img src="https://img.icons8.com/fluency/48/work.png" width="28" align="center" alt="Tools"/> Skills

**Hardware & Embedded**  
![KiCad](https://img.shields.io/badge/KiCad-FFFFFF?style=flat-square&logo=kicad&logoColor=black)
![C/C++](https://img.shields.io/badge/C%2F%2B%2B-00599C?style=flat-square&logo=c%2B%2B&logoColor=white)
![Arduino](https://img.shields.io/badge/-Arduino-00979D?style=flat-square&logo=Arduino&logoColor=white)
![ESP32](https://img.shields.io/badge/ESP--IDF-E7352C?style=flat-square&logo=espressif&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)

**Telecom & Networking**  
![TCP/IP](https://img.shields.io/badge/TCP%2FIP-000000?style=flat-square)
![Wireshark](https://img.shields.io/badge/Wireshark-1679A7?style=flat-square&logo=wireshark&logoColor=white)
![Cisco](https://img.shields.io/badge/Cisco-1BA0D7?style=flat-square&logo=cisco&logoColor=white)
![MQTT](https://img.shields.io/badge/MQTT-660066?style=flat-square&logo=mqtt&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=flat-square&logo=linux&logoColor=black)

**Lab & Test Equipment:**  
<span style="white-space: nowrap;"><img src="https://img.icons8.com/?id=CWZOl3WNER6r&format=png&size=48" width="18" align="center" alt="Oscilloscope"/> Oscilloscope</span> | <span style="white-space: nowrap;"><img src="https://img.icons8.com/?id=ifNCynmGXOFJ&format=png&size=48" width="18" align="center" alt="Multimeter"/> Multimeter</span> | <span style="white-space: nowrap;"><img src="https://img.icons8.com/?id=bQbX7tMYRcoT&format=png&size=48" width="18" align="center" alt="Logic Analyzer"/> Logic Analyzer</span> | <span style="white-space: nowrap;"><img src="https://img.icons8.com/?id=kI89qXDv4f5O&format=png&size=48" width="18" align="center" alt="Signal Generator"/> Signal Generator</span>

---

## <img src="https://img.icons8.com/fluency/48/microscope.png" width="28" align="center" alt="Research"/> Research

- Weather-integrated predictive energy management for hybrid solar.
- MPPT SCC design with INC algorithm.

---

## <img src="https://img.icons8.com/fluency/48/business-contact.png" width="28" align="center" alt="Contact"/> Contact

<p align="left">
  <a href="mailto:mail@smrk.in"><img src="https://img.icons8.com/?id=LPcVDft9Isqt&format=png&size=48" width="32" alt="Email"/></a>
  <a href="mailto:supratimrk@outlook.com"><img src="https://img.icons8.com/?id=ut6gQeo5pNqf&format=png&size=48" width="32" alt="Outlook"/></a>
  <a href="https://smrk.in"><img src="https://img.icons8.com/?id=VJz2Ob51dvZJ&format=png&size=48" width="32" alt="Website"/></a>
  <a href="https://github.com/supratimrk"><img src="https://img.icons8.com/?id=AZOZNnY73haj&format=png&size=48" width="32" alt="GitHub"/></a>
  <a href="https://twitter.com/supratimrk"><img src="https://img.icons8.com/?id=yoQabS8l0qpr&format=png&size=48" width="32" alt="X"/></a>
</p>
